...
Route.prototype.getUrlsByApiId = function(api_id, cb) {
  var client = this.client;
  var table = this.table;
  var source = function(n) {
    if (n.get && n.get.api_id === api_id) {
      return true;
    }
    if (n.post && n.post.api_id === api_id) {
      return true;
    }
    if (n.delete && n.delete.api_id === api_id) {
      return true;
    }
    if (n.put && n.put.api_id === api_id) {
      return true;
    }
  };
  client.findKeys(table, source, function(err, data) {
    if (err) {
      console.log("ERROR: Route.getUrlsByApiId:", err);
      cb(err);
    } else {
      cb(null, data);
    }
